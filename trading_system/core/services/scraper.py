...
def run_op_scraper():
    ...
    try:
        ...
        if len(op_data_objs) > 0:
            ...
            existing_data = OptionData.objects.filter(
                date__in=[item["date"] for item in op_data_objs]
            ).values_list("date", flat=True)
            # transform a existing set
            existing_set = set(existing_data)
            # filter new data
            new_data = [
                item for item in op_data_objs if item["date"] not in existing_set
            ]
            if not new_data:
            ...
            if serializer.is_valid():
                OptionData.objects.bulk_create(
                    [OptionData(**item) for item in serializer.data]
                )
                core_logger.info("Option data successfully saved.")
            else:
                core_logger.error(
                    f"sync option data validation error: {serializer.errors}"
                )
    ...
    try:
        ...
        while current_date.date() <= end_date.date():
            ...
            for market_period in market_periods:
                ...
                if isinstance(market_raw_data, list) and len(market_raw_data) > 0:
                    night_volume = (
                        market_raw_data[0][8] if market_raw_data[0][8] != "-" else "0"
                    )
                    day_volume = (
                        market_raw_data[0][9] if market_raw_data[0][9] != "-" else "0"
                    )
                    market_data_obj = {
                        "year": formatted_target_day.split("/")[0],
                        "month": formatted_target_day.split("/")[1],
        ...
        if len(market_data_objs) > 0:
            ...
            existing_data = PriceData.objects.filter(
                ...
                period__in=[item["period"] for item in market_data_objs],
            ...
            new_data = [
                item
                for item in market_data_objs
                if (item["date"], item["period"]) not in existing_set
            ]
            if not new_data:
            ...
            if serializer.is_valid():
                PriceData.objects.bulk_create(
                    [PriceData(**item) for item in serializer.data]
                )
                core_logger.info("Price data successfully saved.")
            else:
                core_logger.error(
                    f"sync price data validation error: {serializer.errors}"
                )
    except Exception as e:
...
def run_unfulfilled_op_scraper():
    ...
    try:
        ...
        while current_date <= end_date:
            ...
            if isinstance(op_raw_data, list) and len(op_raw_data) > 0:
                raw_data_dict = {
                    "tw_trade_unfulfilled_buy_call_count": int(
                        op_raw_data[0][10].replace(",", "")
                    ),
                    ...
                    "fr_trade_unfulfilled_sell_put_count": int(
                        op_raw_data[5][9].replace(",", "")
                    ),
                    "fr_trade_unfulfilled_sell_put_amount": int(
                        op_raw_data[5][10].replace(",", "")
                    ),
                }
                op_types = ["call", "put"]
                directions = ["buy", "sell"]
                ...
                for op_type in op_types:
                    for direction in directions:
                        for trader in traders:
                            op_data_obj = {}
                            op_data_obj["year"] = int(
                                formatted_target_day.split("/")[0]
                            )
                            op_data_obj["month"] = int(
                                formatted_target_day.split("/")[1]
                            )
                            op_data_obj["date"] = datetime.strptime(
                                formatted_target_day, DATE_FORMAT
                            ).date()
        ...
        if len(op_data_objs) > 0:
            # search for db existing date -> ['date1','date2'...]
            existing_data = UnfulfilledOp.objects.filter(
                date__in=[item["date"] for item in op_data_objs]
            ).values_list("date", flat=True)
            # transform a existing set
            existing_set = set(existing_data)
            ...
            if serializer.is_valid():
                UnfulfilledOp.objects.bulk_create(
                    [UnfulfilledOp(**item) for item in serializer.data]
                )
                core_logger.info("unfulfilled option data successfully saved.")
            else:
                core_logger.error(
                    f"sync unfulfilled option data validation error: {serializer.errors}"
                )
    except Exception as e:
    ...
    try:
            ...
            if isinstance(future_raw_data, list) and len(future_raw_data) > 0:
                ...
                for index, future_data in enumerate(future_raw_data):
                    if future_data[1] == "臺股期貨":
                        raw_data_dict["tw_TX_unfulfilled_count"] = int(
                            future_data[13].replace(",", "")
                        )
                        raw_data_dict["tw_TX_unfulfilled_amount"] = int(
                            future_data[14].replace(",", "")
                        )
                        raw_data_dict["it_TX_unfulfilled_count"] = int(
                            future_raw_data[index + 1][11].replace(",", "")
                        )
                        raw_data_dict["it_TX_unfulfilled_amount"] = int(
                            future_raw_data[index + 1][12].replace(",", "")
                        )
                        raw_data_dict["fr_TX_unfulfilled_count"] = int(
                            future_raw_data[index + 2][11].replace(",", "")
                        )
                        raw_data_dict["fr_TX_unfulfilled_amount"] = int(
                            future_raw_data[index + 2][12].replace(",", "")
                        )
                    elif future_data[1] == "電子期貨":
                        raw_data_dict["tw_TE_unfulfilled_count"] = int(
                            future_data[13].replace(",", "")
                        )
                        raw_data_dict["tw_TE_unfulfilled_amount"] = int(
                            future_data[14].replace(",", "")
                        )
                        raw_data_dict["it_TE_unfulfilled_count"] = int(
                            future_raw_data[index + 1][11].replace(",", "")
                        )
                        raw_data_dict["it_TE_unfulfilled_amount"] = int(
                            future_raw_data[index + 1][12].replace(",", "")
                        )
                        raw_data_dict["fr_TE_unfulfilled_count"] = int(
                            future_raw_data[index + 2][11].replace(",", "")
                        )
                        raw_data_dict["fr_TE_unfulfilled_amount"] = int(
                            future_raw_data[index + 2][12].replace(",", "")
                        )
                    elif future_data[1] == "金融期貨":
                        raw_data_dict["tw_TF_unfulfilled_count"] = int(
                            future_data[13].replace(",", "")
                        )
                        raw_data_dict["tw_TF_unfulfilled_amount"] = int(
                            future_data[14].replace(",", "")
                        )
                        raw_data_dict["it_TF_unfulfilled_count"] = int(
                            future_raw_data[index + 1][11].replace(",", "")
                        )
                        raw_data_dict["it_TF_unfulfilled_amount"] = int(
                            future_raw_data[index + 1][12].replace(",", "")
                        )
                        raw_data_dict["fr_TF_unfulfilled_count"] = int(
                            future_raw_data[index + 2][11].replace(",", "")
                        )
                        raw_data_dict["fr_TF_unfulfilled_amount"] = int(
                            future_raw_data[index + 2][12].replace(",", "")
                        )
                    elif future_data[1] == "小型臺指期貨":
                        raw_data_dict["tw_MTX_unfulfilled_count"] = int(
                            future_data[13].replace(",", "")
                        )
                        raw_data_dict["tw_MTX_unfulfilled_amount"] = int(
                            future_data[14].replace(",", "")
                        )
                        raw_data_dict["it_MTX_unfulfilled_count"] = int(
                            future_raw_data[index + 1][11].replace(",", "")
                        )
                        raw_data_dict["it_MTX_unfulfilled_amount"] = int(
                            future_raw_data[index + 1][12].replace(",", "")
                        )
                        raw_data_dict["fr_MTX_unfulfilled_count"] = int(
                            future_raw_data[index + 2][11].replace(",", "")
                        )
                        raw_data_dict["fr_MTX_unfulfilled_amount"] = int(
                            future_raw_data[index + 2][12].replace(",", "")
                        )
                    elif future_data[1] == "股票期貨":
                        raw_data_dict["tw_SF_unfulfilled_count"] = int(
                            future_data[13].replace(",", "")
                        )
                        raw_data_dict["tw_SF_unfulfilled_amount"] = int(
                            future_data[14].replace(",", "")
                        )
                        raw_data_dict["it_SF_unfulfilled_count"] = int(
                            future_raw_data[index + 1][11].replace(",", "")
                        )
                        raw_data_dict["it_SF_unfulfilled_amount"] = int(
                            future_raw_data[index + 1][12].replace(",", "")
                        )
                        raw_data_dict["fr_SF_unfulfilled_count"] = int(
                            future_raw_data[index + 2][11].replace(",", "")
                        )
                        raw_data_dict["fr_SF_unfulfilled_amount"] = int(
                            future_raw_data[index + 2][12].replace(",", "")
                        )
                    elif future_data[1] == "ETF期貨":
                        raw_data_dict["tw_ETF_unfulfilled_count"] = int(
                            future_data[13].replace(",", "")
                        )
                        raw_data_dict["tw_ETF_unfulfilled_amount"] = int(
                            future_data[14].replace(",", "")
                        )
                        raw_data_dict["it_ETF_unfulfilled_count"] = int(
                            future_raw_data[index + 1][11].replace(",", "")
                        )
                        raw_data_dict["it_ETF_unfulfilled_amount"] = int(
                            future_raw_data[index + 1][12].replace(",", "")
                        )
                        raw_data_dict["fr_ETF_unfulfilled_count"] = int(
                            future_raw_data[index + 2][11].replace(",", "")
                        )
                        raw_data_dict["fr_ETF_unfulfilled_amount"] = int(
                            future_raw_data[index + 2][12].replace(",", "")
                        )
                    elif future_data[0] == "期貨小計":
                        raw_data_dict["tw_subtotal_unfulfilled_count"] = int(
                            future_data[12].replace(",", "")
                        )
                        raw_data_dict["tw_subtotal_unfulfilled_amount"] = int(
                            future_data[13].replace(",", "")
                        )
                        raw_data_dict["it_subtotal_unfulfilled_count"] = int(
                            future_raw_data[index + 1][11].replace(",", "")
                        )
                        raw_data_dict["it_subtotal_unfulfilled_amount"] = int(
                            future_raw_data[index + 1][12].replace(",", "")
                        )
                        raw_data_dict["fr_subtotal_unfulfilled_count"] = int(
                            future_raw_data[index + 2][11].replace(",", "")
                        )
                        raw_data_dict["fr_subtotal_unfulfilled_amount"] = int(
                            future_raw_data[index + 2][12].replace(",", "")
                        )
                ...
                for future_name in future_names:
                    for trader in traders:
                        future_data_obj = {}
                        future_data_obj["year"] = int(
                            formatted_target_day.split("/")[0]
                        )
                        future_data_obj["month"] = int(
                            formatted_target_day.split("/")[1]
                        )
                        future_data_obj["date"] = datetime.strptime(
                            formatted_target_day, DATE_FORMAT
                        ).date()
        ...
        if len(future_data_objs) > 0:
            ...
            existing_data = UnfulfilledFt.objects.filter(
                date__in=[item["date"] for item in future_data_objs]
            ).values_list("date", flat=True)
            # transform a existing set
            existing_set = set(existing_data)
            # filter new data
            new_data = [
                item for item in future_data_objs if item["date"] not in existing_set
            ]
            if not new_data:
            ...
            if serializer.is_valid():
                UnfulfilledFt.objects.bulk_create(
                    [UnfulfilledFt(**item) for item in serializer.data]
                )
                core_logger.info("unfulfilled future data successfully saved.")
            else:
                core_logger.error(
                    f"sync unfulfilled future data validation error: {serializer.errors}"
                )
    except Exception as e:
    ...
    try:
        ...
        for settlement_date in settlement_dates:
            ...
            settlement_date_obj = {
                ...
                "day": WEEKDAY_TRANSFORM[
                    datetime.strptime(settlement_date, DATE_FORMAT).weekday()
                ],
            }
        ...
        if len(settlement_date_objs) > 0:
            ...
            existing_data = Settlement.objects.filter(
                date__in=[item["date"] for item in settlement_date_objs]
            ).values_list("date", flat=True)
            # transform a existing set
            existing_set = set(existing_data)
            # filter new data
            new_data = [
                item
                for item in settlement_date_objs
                if item["date"] not in existing_set
            ]
            if not new_data:
            ...
            if serializer.is_valid():
                Settlement.objects.bulk_create(
                    [Settlement(**item) for item in serializer.data]
                )
                core_logger.info("Settlement date successfully saved.")
            else:
                core_logger.error(
                    f"sync settlement validation error: {serializer.errors}"
                )
    except Exception as e:
        ...
        for op_type in op_types:
            for direction in directions:
                for trader in traders:
                    ...
                    data["unfulfilled_count"] = row[
                        f"{trader}_trade_unfulfilled_{direction}_{op_type}_count"
                    ]
                    data["unfulfilled_amount"] = row[
                        f"{trader}_trade_unfulfilled_{direction}_{op_type}_amount"
                    ]
